...
def create_data(anomaly=False, noise=False):
    ...
    if anomaly:
        ...
        temperatures = base_temperature + np.random.normal(0, temperature_variation, len(days))  # Генерация температур без аномалии

    if noise:
        noise_factor = np.random.uniform(0.1, 0.5)  # Генерация коэффициента шума
        temperatures += np.random.normal(0, noise_factor, len(days))  # Добавление шума к температуре

    return pd.DataFrame({'Day': days, 'Temperature': temperatures})  # Создание DataFrame с днями и температурами

def save_data(data, folder, filename):
    if not os.path.exists(folder):
        os.makedirs(folder)  # Создание директории, если её нет
    data.to_csv(os.path.join(folder, filename), index=False)  # Сохранение данных в CSV файл

# Создание и сохранение данных
for i in range(5):
    data = create_data(anomaly=i % 2 == 0, noise=i % 3 == 0)  # Генерация данных с аномалиями и шумом
    if i < 3:
